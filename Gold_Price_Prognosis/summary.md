...
4. **Evaluation** (Kapitel 5): metrics tables, per-lead-time error comparison, interactive Plotly charts, loss curves, a combined comparison across all 15 models (9 univariate + 6 multivariate), and a single combined CSV artifact (`artifacts/metrics/all_models_comparison.csv`) with every model's error values.
...
| **Chronos (Original, small)** | Amazon's pretrained **zero-shot** time-series foundation model (`amazon/chronos-t5-small`), a T5-architecture transformer trained on a huge corpus of diverse time series, used here with **no training at all**. | Gold has a long, clean, well-behaved public price history, which is exactly the setting where a general-purpose pretrained forecaster is expected to be competitive — this tests whether "generic time-series knowledge" transfers to gold without any gold-specific training at all. |
| **Chronos (Bolt, small)** | A faster, distilled/optimized, quantile-based variant of Chronos (`amazon/chronos-bolt-small`) — also zero-shot. | Same rationale as Chronos Original, but checks whether the faster/quantile-based variant trades away accuracy for speed on this specific series, or holds up just as well. |
| **Chronos (Original, base)** | `amazon/chronos-t5-base` — same T5 architecture and zero-shot approach as the small variant, ~4x more parameters. | Added to test whether model *scale* alone buys accuracy on this series without any gold-specific training — the small/base/large trio isolates the scale effect from the architecture effect. |
| **Chronos (Original, large)** | `amazon/chronos-t5-large` — ~710M parameters, by far the largest model in this project. | Same rationale as base; also the most expensive by a wide margin (~6.5 min for 13 zero-shot inference calls on CPU, vs. ~1-2 min for the small/base variants) since zero-shot models skip HPO and retraining entirely — the cost here is pure model size, not repeated fitting. |
| **Chronos (Bolt, base)** | `amazon/chronos-bolt-base` — larger sibling of Chronos Bolt small, still fast/distilled/quantile-based. | Tests the scale effect specifically within the faster Bolt family, independent of the slower T5 family. |
...
| **Chronos (all 5 variants)** | none — zero-shot, no training | context truncated to last 512 observations; model size is the only thing that differs between variants | `model_id` = `"amazon/chronos-t5-small"` / `-base` / `-large` (Original small/base/large) and `"amazon/chronos-bolt-small"` / `-base` (Bolt small/base) — no HPO ever runs for any of them |
...
- **Training cost profile**: Chronos is the cheapest *category* (no training, ever — just 13 inference calls, one per test window), but scales with model size within that category: Chronos small variants run in ~2–16s total, base variants ~1–2 min, and `chronos-t5-large` (~710M parameters, the largest model in this project) takes ~6.5 minutes on CPU despite still being just 13 forward passes — no HPO, no retraining, purely the cost of one big model's inference. SARIMA/SARIMAX/XGBoost (both variants) are cheap-to-moderate (each of the ~13×26 refits — 25 trials + 1 final, ×13 rolling windows — takes seconds). PatchTST/TFT are the most expensive by far, since even a "warm-start" update still runs several epochs of backprop at every one of ~13 windows, for every one of 25 HPO trials.
...
|---|---|---|---|---|---|
| **Chronos (Original, large)** | **169.60** | 233.24 | 19.70 | 3.90 | 0.456 |
| Naive | 186.04 | 237.41 | 21.61 | 4.32 | 0.410 |
| Chronos (Original, base) | 188.78 | 247.72 | 21.93 | 4.32 | 0.439 |
| Chronos (Bolt, base) | 188.85 | 247.92 | 21.94 | 4.30 | 0.431 |
| SARIMA | 189.53 | 248.61 | 22.02 | 4.34 | 0.423 |
| Chronos (Original, small) | 190.56 | 248.94 | 22.14 | 4.36 | 0.431 |
| Chronos (Bolt, small) | 202.39 | 261.49 | 23.51 | 4.63 | 0.431 |
...
| Chronos (Original, small) | 57.0 | 190.4 | 285.3 |
| Chronos (Original, base) | 56.0 | 212.0 | 266.9 |
| Chronos (Original, large) | 61.0 | **174.2** | **247.5** |
| Chronos (Bolt, small) | 69.0 | 192.4 | 330.2 |
| Chronos (Bolt, base) | 64.1 | 199.0 | 290.4 |
...
- **`chronos-t5-large` is the single best univariate model overall — the only one to beat naive on MAE** (169.60 vs. 186.04, an ~9% reduction), and it's a genuinely zero-shot result: no gold-specific training, no HPO, just a bigger pretrained model. Digging into *where* it wins is the more interesting part: at day 1 it's slightly worse than naive/SARIMA (61.0 vs. 50.3/51.2) — short-horizon persistence is still hard to beat — but at day 10 and day 20 it's clearly ahead of every other univariate model (174.2 and 247.5, vs. naive's 203.0 and 266.5). That's the opposite pattern from the small Chronos variants (Original/Bolt small), which track naive closely at every horizon. **Model scale specifically seems to help medium/long-horizon structure, not short-horizon noise.** The base variants (both families) land in between — close to naive/SARIMA overall, without large's longer-horizon edge — so the gain isn't linear in model size; it shows up mainly at the largest size tested here.
- **The two "base" variants and the small "Original" essentially tie with naive/SARIMA** (188–191 MAE, within ~2% of each other) — reinforcing that most of Chronos's zero-shot competitiveness on this series comes from *not needing gold-specific training* to match a dedicated statistical model, while the *extra* edge over naive specifically required the largest model tested.
